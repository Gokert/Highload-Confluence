...
* 
Необходима проработка архитектуры, которая учитывает отказ компонентов и продолжает стабильно работать некоторое или неограниченное количество времени. Функции с наибольшим трафиком находятся в приоритете, скорее всего при отказе компонентов не получится поддержать функциональность на 100%.
...
Будем использовать API Gateway - Backend for frontend для изолированности логики различных клиентов и для централизованной точки роутинга.

Permission Service - сервис, обеспечивающий авторизацию и права доступа к документам.
