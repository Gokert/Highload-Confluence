...
Механизм копирования: сначала приходит запрос на изменение/создание документа на бэкенд documents, после сохранения документа происходит запрос бэкенду documents-search. Далее происходит асинхронный запрос на запись в kafka, откуда из очереди вынимает бэкенд worker. После осуществляется запрос на запись в бд ElasticSearch
