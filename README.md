...
С помощью Weighted Least Connections входящие запросы распределяются среди служб, запущенных в подах Kubernetes. Отказоустойчивость внутри служб обрабатывается самим Kubernetes.
...
| acl                              | хранения данных о правах пользователя и документа |
