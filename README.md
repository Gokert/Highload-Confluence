...
| Пиковое потребление в течении суток |  2.31 ГБ/сек   |
...
* Graceful degradation: Необходима проработка архитектуры, которая учитывает отказ компонентов и продолжает стабильно работать некоторое или неограниченное количество времени. Функции с наибольшим трафиком находятся в приоритете, скорее всего при отказе компонентов не получится поддержать функциональность на 100%. 
Для Confluence критичной функциональностью будет авторизация, просмотр и редактирование документов. Функциональность, которая имеет право "отвалиться": полнотекстовый поиск по документам.
...
Будем использовать API Gateway - Backend for frontend для централизованной точки роутинга.


Permission Service - сервис, обеспечивающий авторизацию пользователей и хранение их прав.


Documents service - сервис, реализующий создание документов, сохранение картинок, отдачу документов пользователю.
...
Search service - сервис, обеспечивающий поиск по документам через ElasticSearch.
...
Механизм копирования заключается в разделении пайплайнов для чтения и записи в БД. Запись в БД происходит асинхронно - микросервис documents-service кладет данные в виде сообщений в Kafka,
а затем другой микросервис worker достает эти сообщения и записывает данные в ElasticSearch.
...
|:------------|:-----:|:---:|:----:|:---------:|
| Permission  | 127.3 |  2  | 1 Гб | 257 МБ/с  |
| Documents   | 127.3 |  2  | 1 Гб | 2.31 ГБ/с |
| Search      |  9.7  |  1  | 1 Гб | 73 Байт/с |
| API Gateway | 127.3 |  2  | 2 Гб | 2.31 Гб/с |
...
|:--------------|:-------:|:------------------------------:|:-----:|:---:|:-------------------:|:-----------------------------:|
| kube node     |   own   | Xeon E5-2637/2x4Gb/1xNVMe128GB |   4   |  4  |         200         |               3               |
| Redis         |   own   | Xeon E5-2637/2x4Gb/1xNVMe128GB |   4   |  1  |         200         |               3               |
| PostgreSQL    |   own   |   Xeon E5-2637/2x4Gb/4TBNVMe   |   4   |  2  |         350         |               6               |
| ElasticSearch |   own   |   Xeon E5-2637/2x4Gb/1TBNVMe   |   4   |  1  |         300         |               5               |
| ClickHouse    |   own   |  Xeon E5-2637/2x4Gb/8x4TBNVMe  |   4   |  1  |        1350         |              23               |
| CEPH          |   own   |  Xeon E5-2637/2x4Gb/6x4TBNVMe  |   4   |  1  |        1050         |             17.5              |
